...
if(isset($_POST['update_issue_btn'])){

    $IStatus = validate($_POST['issue_status']);
    $bookingId = validate($_POST['booking_id']);

    
    $query = "UPDATE bookings SET issue_status='$IStatus' WHERE booking_no='$bookingId' ";
    $result = mysqli_query($conn, $query);

    if($result){
        redirect('booking-view.php?booking_id='.$bookingId,'Booking Issue status updated Successfully');
    }else{
        redirect('booking-view.php?booking_id='.$bookingId,'Something Went Wrong');
    }
}
